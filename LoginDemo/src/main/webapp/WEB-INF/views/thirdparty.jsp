					...
					<c:forEach items="${documents}" var="order" varStatus="loop">
						...
						<c:if test="${loop.index == loop2.index }">
							...
							<c:if test="${order.getStatus() == 'Rejected'}">
								...
								<td>${updated}</td>
								<td><img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer;" ng-click="initMap('${order.getTruckId()}');"/></td>
							</c:if>
						</tr>
						</c:if>
						</c:forEach>
...
<!-- 												<option value="Reason3">Reason code 3</option> -->
									...
									<!-- <label class="control-label" for="Stausupdate">Status</label> 
								<select name="status"
									id="status" class="form-control">
									<option value="Initiated" selected="selected">Initiated</option>
									<option value="Processed">Processed</option>
									<option value="Rejected">Rejected</option>
								</select> -->
								</div>
								<!-- 	<div class="form-group">
								<label for="updatecomments">Comments</label> <input type="text"
									class="form-control" id="updatecomments"
