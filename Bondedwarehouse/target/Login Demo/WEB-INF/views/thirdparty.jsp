...
<div id="light" class="white_content" $("body").css("overflow", "hidden");>
   ...
   <div class="TruckStatus" id="truckStatus">Truck Status: Left Factory</div>
   <div id="noData" style="display:none; font-weight:bold; font-size:18px;">No GPS data available!</div>
   <div id="mappop">
   </div>
   </div>
		...
		<c:if test="${param.statusUpdate == 'success' || statusUpdate == 'success'}">
			...
			<c:if test="${highpriorityDocListCount!=''}">
				<div class="alertHigh alert-info">
						<strong><img title="High Alert" style="width:20px; height:20px;" src="./resources/img/alert.gif"/><a href="./thirdpartyLogin">  Number of truck(s) arriving to custom: ${highpriorityDocListCount}</a></strong>
				</div>
			</c:if>
			<div class="container">
				<c:choose>
				...
				<table class="table table-bordered display" id="datatable">
					...
					<tbody>
						...
						<tr>
								...
								<td>
								<c:if test="${prioritydoc.getStatus() == 'Initial'}">
								    ...
								    <img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${prioritydoc.getTruckId()}');"/>						
								</td>
						</tr>
						</c:forEach>
					</tbody>
				...
				<table class="table table-bordered display" id="datatable">
					...
					<c:if test="${order.getStatus() == 'Rejected'}">
						...
						<tr>
							...
							<td><img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${order.getTruckId()}');"/></td>
						</tr>
						</c:if>
					</c:forEach>
						...
						<c:if test="${order.getStatus() != 'Rejected'}">
							...
							<tr>
								...
								<td>${order.getStatus()}</td>
								<td>${order.getEstimatedTime()}</td>
								<c:if test="${order.getStatus() == 'Initial'}">
								   <td>
										<span id="update_button"
											class=" update_button btn" data-toggle="modal"
											data-target="#updateStatus1" title="Update Status" style="margin-top:8px;"
								        ...
								        <img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${order.getTruckId()}');"/>
								  </td>
								</c:if>
								<c:if test="${order.getStatus() != 'Received' && order.getStatus() != 'Initial'}">
								  <td>
									   <img ng-src="./resources/img/truck-search.png" title="Locate the truck" alt="Truck Track" style="width:50px; height:50px; cursor:pointer; float:right;" ng-click="initMap('${order.getTruckId()}');"/>
							      </td>
							    </c:if>
